...
async function confidentialTransfer(
    ...
    let ct = await owner.encryptValue(BigInt(transferAmount), await token.getAddress(), selector)
    const ciphertexts = ct.map((val) => val.ciphertext);
    const signatures = ct.map((val) => val.signature);

    await (await func(alice.address, ciphertexts[0], signatures[0], false, {gasLimit})).wait()
...
async function confidentialApprove(
    ...
    const selector = func.fragment.selector
    let ct = await owner.encryptValue(BigInt(BigInt(approveAmount)), await token.getAddress(), selector)
    const ciphertexts = ct.map((val) => val.ciphertext);
    const signatures = ct.map((val) => val.signature);

    await (await func(alice.address, ciphertexts[0], signatures[0], {gasLimit})).wait()
    await assertAllowance(token, approveAmount, owner, alice.address)
}
...
async function confidentialTransferFrom(
    ...
    await (await func(owner.address, alice.address, ciphertexts[0], signatures[0], false, {gasLimit})).wait()
